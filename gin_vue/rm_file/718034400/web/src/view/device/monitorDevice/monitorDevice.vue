...
<template>
  ...
  <el-input v-model="searchInfo.streamUrl" placeholder="搜索条件" />
</el-form-item>
  ...
  <el-select v-model="searchInfo.status" clearable filterable placeholder="请选择" @clear="()=>{searchInfo.status=undefined}">
    <el-option v-for="(item,key) in Equipment statusOptions" :key="key" :label="item.label" :value="item.value" />
  </el-select>
</el-form-item>
    ...
    {{ filterDict(scope.row.status,Equipment statusOptions) }}
    </template>
</el-table-column>
...
// 自动化生成的字典（可能为空）以及字段
const Equipment statusOptions = ref([])
const formData = ref({
            deviceName: '',
            groupId: undefined,
            streamUrl: '',
...
const setOptions = async () =>{
    Equipment statusOptions.value = await getDictFunc('Equipment status')
}
